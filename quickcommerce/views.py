...
# Global variable to temporarily store fetched data
fetched_data = []
...
def trigger_fetch(request):
    ...
    """
    global fetched_data
    result = fetch_api_data.delay()  # Run task in the background
    fetched_data = result.get(timeout=120)  # Wait for task to complete (max 2 minutes)
    return JsonResponse({"status": "Data fetch completed successfully"})

def download_csv(request):
    """
    Convert fetched JSON data to a CSV and return it for download.
    """
    global fetched_data
    if not fetched_data:
        return JsonResponse({"error": "No data available. Fetch the data first."})
    ...
    response["Content-Disposition"] = 'attachment; filename="fetched_products.csv"'
